...
export class MonobankClient {
  ...
  /** Signing keys registered for the monoPay button widget. */
  async monopayPubkeyList(): Promise<
    { keyId: string; keyValue: string; keyName?: string; expiresAt?: string }[]
  > {
    const { result } = await this.request<{
      result: { keyId: string; keyValue: string; keyName?: string; expiresAt?: string }[]
    }>("/api/merchant/monopay/pubkey-list")
    return result ?? []
  }
...
/**
 * Signature for the monoPay button widget, exactly per the official docs
 * (docs--signature-example): SHA-256 over `JSON.stringify(orderData) + requestId`,
 * signed with the merchant's ECDSA P-256 private key, DER-encoded, base64.
 *
 * The private key pair is generated by the merchant; its PUBLIC part is
 * registered with Monobank (keyId comes from GET /api/merchant/monopay/pubkey-list).
 */
export function signMonoPayPayload(
  ...
  sign.update(orderJson + requestId)
  // Node emits DER-encoded ECDSA signatures by default — matches the widget.
  return sign.sign(privateKeyPem).toString("base64")
}

/** MONOPAY_PRIVATE_KEY may hold raw PEM or base64-wrapped PEM (easier in .env). */
export function resolveMonoPayPrivateKey(): string | null {
  const raw = process.env.MONOPAY_PRIVATE_KEY
  if (!raw) return null
